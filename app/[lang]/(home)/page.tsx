...
import { ArrowRight, GitFork, Heart, Star, Tag } from 'lucide-react';
import { GitHubIcon, TelegramIcon } from '@/components/icons';
import { Logo } from '@/components/logo';
import { Features } from '@/components/home/features';
import { InstallCommand } from '@/components/home/install-command';
import { getGitHubStats, formatCount } from '@/lib/github-stats';
import { i18n } from '@/lib/i18n';
import { appName, productRepoUrl, telegramChannelUrl, donateUrl } from '@/lib/shared';
    ...
    <main className="flex flex-1 flex-col">
      ...
      <section className="relative overflow-hidden border-b">
        ...
        <div className="mx-auto flex w-full max-w-5xl flex-col items-center px-4 py-20 text-center sm:py-28">
          ...
          {/* Live build-time stats */}
          <dl className="mt-8 flex flex-wrap items-center justify-center gap-x-8 gap-y-3 text-sm">
            <Stat icon={<Star className="size-4" aria-hidden />} label={m.stars}>
              {formatCount(stats.stars)}
            </Stat>
            <Stat icon={<GitFork className="size-4" aria-hidden />} label={m.forks}>
              {formatCount(stats.forks)}
            </Stat>
            <Stat icon={<Tag className="size-4" aria-hidden />} label={m.latest}>
              {stats.latestVersion}
            </Stat>
          </dl>
        </div>
